...
reachable at `http://<judge-host>:11434`, and the session datasets present at
...
1. Start the AXON server with the session harness environment:
   ...
   ```bash
   AXON_MAX_PRE_SEND_TOKENS=32000 AXON_COMPLETION_MODEL="ollama/llama3.1:8b" AXON_PROVIDER_OLLAMA=1 OLLAMA_BASE_URL="http://<judge-host>:11434" ~/.pyenv/versions/clock/bin/axon serve-http --port 8765
   ```
